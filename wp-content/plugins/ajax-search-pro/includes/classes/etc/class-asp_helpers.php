...
if (!class_exists("ASP_Helpers")) {
    ...
    class ASP_Helpers {
        ...
        public static function toQueryArgs($search_id, $o) {
            ...
            $args["_exact_matches"] = isset($o['set_exactonly']) ? 1 : $sd['exactonly'];
            ...
            if ($o === false) {
                if ( $sd['searchinposts'] == 1 )
                    $args['post_type'][] = "post";
                if ( $sd['searchinpages'] ) {
                    if ( $sd['exclude_page_parent_child'] != '' )
                        $args['_exclude_page_parent_child'] = str_replace( '|', ',', $sd['exclude_page_parent_child'] );
                    else
                        $args['post_type'][] = "page";
                }
                ...
                foreach ( $args['post_type'] as $kk => $vv) {
                    if ( $vv == "page" && $sd['exclude_page_parent_child'] != '' ) {
                        $args['_exclude_page_parent_child'] = str_replace( '|', ',', $sd['exclude_page_parent_child'] );
        ...
        private static function toQueryArgs_Taxonomies($sd, $o) {
            ...
            $sd['selected-excludeterms'] = w_isset_def( $sd['selected-excludeterms'], array() );
            $sd['selected-excludecategories'] = w_isset_def( $sd['selected-excludecategories'], array() );

            if ( count($sd['selected-excludecategories']) > 0 ) {
                $sd['selected-excludeterms']['category'] = $sd['selected-excludecategories'];
            }

            if ( count( $sd['show_terms']['terms'] ) > 0 ||
                count( $sd['selected-excludeterms'] ) > 0 ||
                count( $o['termset'] ) > 0
            ) {
                ...
                $taxonomies = array_unique(array_keys($sd['selected-excludeterms']));
                ...
                foreach ($taxonomies as $taxonomy) {
                    ...
                    $exclude_t = isset( $sd['selected-excludeterms'][$taxonomy] ) ? $sd['selected-excludeterms'][$taxonomy] : array();
