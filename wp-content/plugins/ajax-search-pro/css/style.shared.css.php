...
<?php echo $asp_div_ids; ?> .probox .proinput {
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['inputfont']); ?>
    line-height: normal;
  flex-grow: 1;
  order: 5;
  /* Ipad and stuff.. */
...
<?php echo $asp_div_ids; ?> .probox .proinput input.orig::-webkit-input-placeholder {
    <?php echo preg_replace(array("/(line-height\:.*?)\;/", "/(\-\-g\-\-)(.*?)\;/"), array('', "'$2';"), $style['inputfont']); ?>
    opacity: 0.85;
}
...
<?php else: ?>
	...
	<?php endif; ?>
<?php endif; ?>
	<?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", w_isset_def( $style['search_text_font'], "") ); ?>
...
<?php echo $asp_res_ids; ?> .results .asp_nores .asp_keyword {
    padding: 0 6px;
    cursor: pointer;
    <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['descfont']); ?>
    font-weight: bold;
}
...
<?php echo $asp_res_ids; ?> .results .item .asp_content h3 {
  ...
  display: inline-block;
  line-height: inherit;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['titlefont']); ?>
}
...
<?php echo $asp_res_ids; ?> .results .item .etc .asp_author {
  padding: 0;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['authorfont']); ?>
}
...
<?php echo $asp_res_ids; ?> .results .item .etc .asp_date {
  margin: 0 0 0 10px;
  padding: 0;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['datefont']); ?>
}
...
<?php echo $asp_res_ids; ?> p.showmore {
  ...
  padding: 10px 5px;
  margin: 0;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['showmorefont']); ?>
}
...
<?php echo $asp_res_ids; ?> .asp_group_header {
  ...
  position: relative;
  z-index: 1000;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['groupbytextfont']); ?>
}
...
<?php echo $asp_set_ids; ?>.searchsettings .asp_label {
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['settingsdropfont']); ?>
}
...
<?php echo $asp_set_ids; ?>.searchsettings fieldset legend {
  ...
  margin: 0;
  background: transparent;
  <?php echo preg_replace("/(\-\-g\-\-)(.*?)\;/", "'$2';", $style['exsearchincategoriestextfont']); ?>
}